...
parser.add_option("-d", "--detallenotaventa", help="Asigna numero de detalles de nota de venta", type="int", default=500)
...
def generarNotaVenta():
	...
	sql = """INSERT INTO nota_venta (id_cliente, fecha_venta, vendedor_id, tienda_id) 
	             VALUES (%d, now() + interval '%d minute', %d, %d);""" %(id_cliente, random.randint(1, 30), vendedor_id, tienda_id)
	cursor.execute(sql)
	conn.commit() 
	cursor.close()
	conn.close()
...
def generarDetalleNotaVenta():
	conn = getConexion()
	cursor = conn.cursor()
	while True:
		cursor.execute("SELECT min(nota_venta_id), max(nota_venta_id) FROM nota_venta;")
		rows = cursor.fetchall()
		nota_venta_id = random.randint(rows[0][0], rows[0][1])
		sql = """SELECT tienda_id 
		             FROM nota_venta
...
def main():
	...
	for i in range(0, options.detallenotaventa):
		generarDetalleNotaVenta()
		print "Detalles de nota de venta %d de %d generadas\n" %(i + 1, options.detallenotaventa)
